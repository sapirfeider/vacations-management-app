...
async function addVacaction(body) {
    ...
    const [rows] = await global.connection.execute(addVacationQuery, [destination, description, image, check_in_date, check_out_date, price])
    console.log(rows)
...
async function updateVacation(body) {
    ...
    //const connection = await getConnection();
    const [rows] = await global.connection.execute(updateQuery, [description, destination, image, checkIn, checkOut, price, id])
    console.log(rows)
    return rows.affectedRows;
}
